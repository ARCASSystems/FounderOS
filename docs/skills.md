...
- **Reads.** `core/working-preferences.md` first, as a gate on its own questions (an Active row saying never to ask about something removes it from the candidate list), then `brain/needs-attention.md` (and `brain/needs-input.md`, its `/dream`-written sibling), `cadence/queue.md`, `cadence/weekly-commitments.md`, `cadence/daily-anchors.md`, `brain/flags.md`, `brain/unconfirmed-facts.md`, `brain/employees.md` (refreshed first via the verdict script so a due review actually surfaces), and yesterday's coach line in `brain/log.md`. It also reads the `seats:` line of `brain/knowledge/*.md` frontmatter - only that line, never a note body - to find notes nobody has routed yet.
- **Writes.** Whichever files your answers belong to (the queue, the commitments, the flags, the ledgers, `core/working-preferences.md` when you promote or drop a proposed row, and a knowledge note's `seats:` field when you answer a routing question), plus one `#morning-loop` entry in `brain/log.md` carrying the coach line and one run line in `brain/agent-runs.jsonl`. It closes the source that raised each question in the same pass.
- **The knowledge sweep.** Its lowest-priority question, ranked behind everything that moves real business state: one untagged knowledge note per run, only when a slot is free after the real asks, and never at stage 1 of a fresh install. A yes routes the note to one seat and re-runs `agents_sync apply`; a no writes `seats: none` so the note is never raised again. A note already routed, or already declined, is never a candidate.
- **Voice rules.** No.
...
- **Reads.** `brain/knowledge/`, `brain/.snapshot.md` if present, and `roles/employees.yaml` if it exists, to see whether any seat's job plainly covers the note. Source from prompt or path.
- **Writes.** `brain/knowledge/<topic>.md` (creates or appends), `brain/knowledge/README.md` (index row), and the note's own `seats:` field once you answer the routing question.
- **Routing.** After the note is written it proposes AT MOST one seat in one line: "this reads like it belongs to the account manager - route it there?". A yes writes `seats: <id>` and runs `python scripts/agents_sync.py apply`, so that seat's read-list carries a pointer to the note - id, topic and path, never the body. A no writes `seats: none`, the tombstone that records the decline so nothing raises the note again. No obvious match means no question at all, and nothing is ever tagged as a side effect of capturing.
- **Voice rules.** No.
- **Prereqs.** `founder-os-setup` complete.

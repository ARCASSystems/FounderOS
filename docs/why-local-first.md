...
- **Your data never leaves the laptop.** The brain is plain markdown files on your disk. Reading them into a session sends them to the model under your subscription's terms, the same as any Claude conversation - but no product server, no telemetry, no sync service ever sees them. Deleting the OS is deleting a folder.

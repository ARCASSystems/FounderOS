...
The skill registry is `skills/index.md` (92 skills, 43 commands); the long-form per-skill reference is `docs/skills.md`. Read those for the lists rather than expecting them duplicated here.

---

## Cross-agent deltas (what differs from a Claude Code session)

- **Slash commands are Claude Code specific.** The 43 commands in `.claude/commands/` cannot be run by a non-Claude agent. Treat them as documented procedures, not executable actions.

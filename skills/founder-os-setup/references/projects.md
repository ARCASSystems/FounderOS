...
4. **`modules`** - `yes` or `no` per module, set from what this setup actually built:
   ...
   - `employees`: `yes` on any normal setup. Phase 2.2 copies the org chart with its five starter roles and step 2.6 of the post-setup tour introduces them by name, so this part of the OS is something the founder has already been shown. Set it `no` only if they said they do not want it and you skipped that step.

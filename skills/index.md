...
26 skills included in v1.3. The setup wizard (`founder-os-setup`) is the entry point.
...
This plugin ships twelve slash commands:
...
26 skills. Each skill is generic: no founder-specific references, no personal names. Voice-neutral for adaptation by the setup wizard using the founder's identity, voice profile, and brand profile.

The three voice-coupled skills (`linkedin-post`, `client-update`, `proposal-writer`) and the readiness-check skill were added in v1.2. The `ingest` and `lint` skills (Karpathy-pattern wiki ops) were added in v1.3 - both additive, no existing skill behavior changes.

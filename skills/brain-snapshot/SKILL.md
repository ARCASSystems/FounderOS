...
name: brain-snapshot
description: >
  Generates and documents the runtime context payload that other skills read at task time. Use this skill when you need to refresh the snapshot manually, when you are wiring a new skill to the snapshot, or when you want to understand what fields skills can rely on. The snapshot is generated by `scripts/brain-snapshot.py` and lives at `brain/.snapshot.md` (gitignored, per-user).

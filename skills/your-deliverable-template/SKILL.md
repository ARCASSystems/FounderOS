...
Read this entire file before writing any document-producing code. Then load the right brand profile per the routing below.
...
The schema is the same in both files. The rest of this skill applies uniformly regardless of which profile was loaded.
...
The active profile is either `core/brand-profile.yml` (operator) or `brands/<slug>/visual.yml` (named brand), chosen per the routing block above. Both files share the same schema and contain:
...
1. Run brand routing (see top of file). Read the chosen profile - `core/brand-profile.yml` for operator, `brands/<slug>/visual.yml` for a named brand. If not set up, stop and ask.

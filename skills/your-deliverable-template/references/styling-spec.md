# Styling spec - page, color, fonts, logo, layout

Load this when you are generating a document and need the visual specs. Apply these rules plus the brand-specific values from the active profile (`core/brand-profile.yml` for operator, `brands/<slug>/visual.yml` for a named brand).
...
The active profile is either `core/brand-profile.yml` (operator) or `brands/<slug>/visual.yml` (named brand), chosen per the routing block in SKILL.md. Both files share the same schema and contain:

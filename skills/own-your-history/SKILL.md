...
3. Own the data: run the remote-safety check from step 4a (a fresh `git init` has no remotes, so this is instant), then copy `templates/operator.gitignore` over `.gitignore`. If `templates/` is not in the OS folder (Plugin install), the file ships with the engine instead - take the first of these that resolves: `${CLAUDE_PLUGIN_ROOT}/templates/operator.gitignore` (Claude Code sets that variable for a plugin's own components; `echo "$CLAUDE_PLUGIN_ROOT"` reads it), then the newest match for `~/.claude/plugins/**/templates/operator.gitignore`. Search rather than assume a fixed plugin path - the layout moves between plugin-manager versions, and a wrong guess reads as a missing file on a healthy install. The shipped `.gitignore` is the developer one that ignores the founder's own files; the operator one tracks their data and ignores only secrets, the private-name patterns file, runtime state, and per-machine settings. Without this swap, "full version history" silently records only the engine. On a plugin-path install there may be no `.gitignore` at all yet - the copy still runs; with no gitignore, secrets and runtime state would enter history on the very first save.

   If neither lookup finds it, STOP before step 5 and record nothing. A first save with no gitignore writes secrets and machine state into history permanently, and no later edit takes them back out. Say exactly: `Version history is on, but I could not find the file that tells it what to leave out, so I have not recorded anything yet. Restart Claude Code and say "own my history" again - that reloads what is installed.`
4. Wire the privacy guard BEFORE the first commit: `git config core.hooksPath .githooks`. Confirm `scripts/private-name-patterns.txt` exists, and leave it empty by default. Do NOT write the founder's own name as a pattern. Their name lives in `core/identity.md`, the names glossary, and the company key-people files, all tracked by design once they own their history, so their own name as a blocked pattern makes every identity-touching commit fail, starting with the one on the next line. What protects them from publishing their data is the remote-safety guard in step 4a, not a block on their own name. The pattern file is for names that must never enter their files at all - a client under NDA, a person they capture notes about off-system - so offer it that way and expect a no: "There is a privacy guard that can block specific names from ever entering your version history - useful for client names under NDA. Want any names in it now? Most people skip this." If a pattern IS written, write `\b` as a literal two-character backslash-b using a file-write tool, never a shell echo (`printf`/`echo -e` turns it into a backspace byte and ships a dead guard that looks installed and matches nothing).

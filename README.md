...
**It learns how you want to be worked with, in a file you can edit.** The chat app you came from has memory, so "it remembers you" is not a reason to install anything. The difference here is where the memory lands. `core/working-preferences.md` is read **before** an answer is produced, the same way your voice profile is read before anything is written - not recalled afterwards, at a model's discretion, out of a store you cannot open.

It ships empty and fills from your own corrections. Say "too long" or "you asked me that already" or "just pick one", and the OS fixes it in that reply and then asks, in one line, whether to stop doing it for good. On your yes it becomes a row with the date and your exact words next to it. Nothing lands without that yes, and anything `/dream` finds in a rant waits in a Proposed table until you say so. The bar it is built to clear: **you should never have to give the same correction twice.** If you do, that is a bug, and the file is right there to check.

**The team keeps its own record.** The five starter roles in `roles/employees.yaml` each write one line about themselves when they run: what they read, what they produced, how it ended, what they could not do. So a review can say this job ran fourteen times and you watched three, instead of reading only the runs you happened to catch. A run line never claims the output was good - that is still your one-word verdict.

**The team grows through one door, sized honestly.** Say "this keeps eating my week" and the Chief of Staff walks a six-rung ladder - an answer, a written rule, a skill, a script, a chartered seat, a team - and proposes the smallest shape that removes the job, with one line on what the bigger option would have bought. Every hire arrives with a passport before your yes: what it may touch, what it never does, what it costs, what leaves the machine, how to fire it. Seats become real, addressable agents generated from the same registry row the audit reads, so you manage a team member by editing their contract, not by learning a second system - and nothing from outside the OS is ever installed without its source shown and your yes given. Their read-list grows too: tag a knowledge note with the seat it belongs to and that seat starts each run knowing about it - a pointer to the note, never its contents, and only ever on your yes. Decline once and the note is marked reviewed, so it never gets raised at you again.
...
Version 1.54.0. Public release. 95 skills, 45 commands, 867 tests. Every push to main runs three CI gates (doc and install parity, the privacy guardian, the LinkedIn pack acceptance suite) and a weekly integrity audit runs on top. The maintainer's full test suite runs upstream before anything lands here; it is not shipped in this repo, so the badge row above is the claim you can verify.

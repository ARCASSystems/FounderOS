...
Run `python scripts/where.py --days 14` first, then answer in two or three plain sentences leading with the work, not the location. Widen the window if nothing looks familiar. If any folder comes back NOT BACKED UP, say so even though they did not ask - that is the more urgent finding. Close by offering to open the folder.

...
- `clients/` (all files)
- `companies/` (all files)
- `roles/` (the employee registry - operator-owned statuses, last_review stamps, and custom rows)
- `system/` (the quarantine catch-net - this install's own failure record)
- any company or project folder setup created at the OS root (arbitrary names; a root path not on the System Layer list is the founder's)
...
- network/, clients/, companies/, roles/, system/
- MEMORY.md (and memory/), stack.json, os-config.yaml
- any company or project folders setup created at the root
